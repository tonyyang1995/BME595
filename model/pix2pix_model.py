...
class Pix2PixModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        #self.loss_names += ['0', ]
        self.loss_names += ['G_CE', 'G_L1_max', 'G_L1_reg', 'G_fake_real',]
    ...
    def set_input(self, input):
        ...
        self.real_B_enc = utils.encode_ab_ind(self.real_B[:, :, ::4, ::4], self.opt)
    ...
    def forward(self):
        ...
        self.fake_B_dec_max = self.netG.module.upsample4(utils.decode_max_ab(self.fake_B_class, self.opt))
        self.fake_B_distr = self.netG.module.softmax(self.fake_B_class)

        self.fake_B_dec_mean = self.netG.module.upsample4(utils.decode_mean(self.fake_B_distr, self.opt))
    ...
    def get_current_visuals(self):
        ...
        visual_ret['gray'] = utils.lab2rgb(torch.cat((self.real_A.type(torch.cuda.FloatTensor), torch.zeros_like(self.real_B).type(torch.cuda.FloatTensor)), dim=1), self.opt)
        visual_ret['real'] = utils.lab2rgb(torch.cat((self.real_A.type(torch.cuda.FloatTensor), self.real_B.type(torch.cuda.FloatTensor)), dim=1), self.opt)

        visual_ret['fake_max'] = utils.lab2rgb(torch.cat((self.real_A.type(torch.cuda.FloatTensor), self.fake_B_dec_max.type(torch.cuda.FloatTensor)), dim=1), self.opt)
        ...
        visual_ret['fake_reg'] = utils.lab2rgb(torch.cat((self.real_A.type(torch.cuda.FloatTensor), self.fake_B_reg.type(torch.cuda.FloatTensor)), dim=1), self.opt)

        visual_ret['hint'] = utils.lab2rgb(torch.cat((self.real_A.type(torch.cuda.FloatTensor), self.hint_B.type(torch.cuda.FloatTensor)), dim=1), self.opt)

        visual_ret['real_ab'] = utils.lab2rgb(torch.cat((torch.zeros_like(self.real_A.type(torch.cuda.FloatTensor)), self.real_B.type(torch.cuda.FloatTensor)), dim=1), self.opt)

        visual_ret['fake_ab_max'] = utils.lab2rgb(torch.cat((torch.zeros_like(self.real_A.type(torch.cuda.FloatTensor)), self.fake_B_dec_max.type(torch.cuda.FloatTensor)), dim=1), self.opt)
        ...
        visual_ret['fake_ab_reg'] = utils.lab2rgb(torch.cat((torch.zeros_like(self.real_A.type(torch.cuda.FloatTensor)), self.fake_B_reg.type(torch.cuda.FloatTensor)), dim=1), self.opt)
